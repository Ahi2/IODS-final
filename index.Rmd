...
# 2.5. Predicting LDA
Now that we have performed LDA, it is time to predict the values based on the model. We already created the test set to be used for predicting the model. It is a new dataset here, which can tell us how good the model fits our data. 
...
The above results reveal that in most cases the GNI classes have been predicted correctly, particularly in low and high GNI classes. There are some discripancy in med_low and Med_high, but generally, we can conclude that other variables such as 'the ratio of Female and Male populations with secondary education' (Edu2.FM) and 'Percetange of female representatives in parliament' (Parli.F) predict classes of GNI very well. The results indicate that the higher the values of these two variables the higher the GNI classes, meaning that the country is wealthier when females participate more in education and when they have a more pronounced role in a given country parliament. 
